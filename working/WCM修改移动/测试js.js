...
var sku = {
    ...
    areaStore: function(params) {
        ...
        $.ajax({
            ...
            success_cb: function(data) {
                if (data.isSuccess) {
                    if (data.data < 1) {
                        $(".js_s_areaStore").html("无库存").show();
                    } else {
                        $(".js_s_areaStore").html("有库存").show();
                    }
                }
            }
        });
    ...
    isExchange: function(productId) {
        ...
        $.ajax({
            ...
            success_cb: function(data) {
                if (data == true) {
                    $(".js_s_price1").filter(".box3").removeClass("s_none");
                    $(".js_s_price1").filter(".box1").addClass("s_none");
                    $(".js_s_price1").filter(".box2").addClass("s_none");
                        ...
                        if(flag!=""){
                            ...
                            if ($('#js-preSaveOrder').attr('skuId')) {
                                if (!istrsidssdssotoken()) {
                                    ...
                                    window.location.href = formSureUrl + "?productId=" + productId + "&skuId=" + $('#js-preSaveOrder').attr('skuId') + "&buyNum=" + $('#js-buyNum').text() + "&activityGoodsId=" + $('#js-preSaveOrder').attr('activityGoodsId') + "&activitySkuId=" + $('#js-preSaveOrder').attr('activitySkuId') + "&activityType=" + $('#js-preSaveOrder').attr('activityType') + "&payType=" + $('#js-preSaveOrder').attr('payType') + "&skuStockMgr=" + skuStockMgr + "&area=" + $('.js_getProvince').find("option:selected").html() + " " + $('.js_getCity').find("option:selected").html() + " " + $('.js_getCityArea').find("option:selected").html();
